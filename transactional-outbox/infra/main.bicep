...
metadata description = 'Deploys the transactional-outbox sample: a serverless, keyless Azure Cosmos DB account plus an App Service web front end that hosts the change feed relay. The deploying user is also granted data-plane access so the optional console app can be run locally.'
