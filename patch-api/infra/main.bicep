...
metadata description = 'Deploys the patch-api sample: a serverless, keyless Azure Cosmos DB account plus an App Service web front end that demonstrates partial document updates (the Patch API). The deploying user is also granted data-plane access so the optional console app can be run locally.'
