...
/// <summary>
/// Builds a <see cref="CosmosClient"/> the same way as the rest of this repository: keyless
/// (DefaultAzureCredential) when no key is supplied, or key-based otherwise. When the endpoint is
/// the local emulator (localhost), it uses Gateway mode and accepts the emulator's self-signed
/// certificate — this only ever applies to a local emulator endpoint.
/// </summary>
