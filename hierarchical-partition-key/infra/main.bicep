...
metadata description = 'Deploys the hierarchical-partition-key sample: a serverless, keyless Azure Cosmos DB account with a hierarchically-partitioned container (/tenantId/userId) and a single-key comparison container, plus an App Service web front end. The deploying user is also granted data-plane access so the optional console app can be run locally.'
